        ...
        if kwargs is not None:
            ...
            for key, value in kwargs.items():
                ...
                if key == "_list":
                    ...
                    raise Exception(
                        "ERROR in the namelist: cannot define `" + key + "` in block " + cls.__name__ + "()");
                else:
...
class Main(SmileiSingleton):
    ...
    def __init__(self, **kwargs):
            ...
            if Main.timestep_over_CFL is None:
                ...
                if Main.maxwell_solver == 'Yee':
                    if (Main.geometry == "AMcylindrical"):
                        Main.timestep = Main.timestep_over_CFL / math.sqrt(
                            1. / Main.cell_length[0] ** 2 + ((Main.number_of_AM - 1) / Main.cell_length[1]) ** 2)
                    else:
        ...
        if (len(Main.grid_length + Main.number_of_cells) == 0
            ...
            raise Exception(
                "Main: you must define two (and only two) between grid_length, number_of_cells and cell_length")
        ...
        if len(Main.number_of_cells) == 0:
            ...
            if smilei_mpi_rank == 0:
                ...
                if any(different):
                    print("\t[Python WARNING] Main.grid_length=" + str(Main.grid_length) + " (was " + str(
                        old_grid_length) + ")")
...
_test_mode = True
smilei_version = '4.7-241-gcdfb396-master'
smilei_mpi_size = 4
...
def trapezoidal(max,
    ...
    def trapeze(max, vacuum, plateau, slope1, slope2):
        def f(position):
            ...
            elif position < vacuum + slope1:
                return max * (position - vacuum) / slope1
            # density plateau
            elif position < vacuum + slope1 + plateau:
                return max
    ...
    def gauss(max, vacuum, length, sigma, center, order):
        def f(position):
            ...
            # gaussian
                ...
                return max * math.exp(-(position - center) ** order / sigma)
            # beyond
            else:
                return 0.0
    ...
    for k, a in kwargs.items():
        if k == "x0":
            ...
            if Main.geometry == "1Dcartesian":
                ...
                if len(a) != order + 1:
                    raise Exception(
                        "2D polynomial profile must have " + str(order + 1) + " coefficients at order " + str(order))
            elif Main.geometry == "3Dcartesian":
                if len(a) != (order + 1) * (order + 2) / 2:
                    raise Exception("3D polynomial profile must have " + str(
                        (order + 1) * (order + 2) / 2) + " coefficients at order " + str(order))
    if Main.geometry == "1Dcartesian":
        def f(x):
            r = 0.
            xx0 = x - x0
...
def tgaussian(start=0., duration=None, fwhm=None, center=None, order=2):
    ...
    def f(t):
        if t < start:
            ...
            return math.exp(-(t - center) ** order / sigma)
        else:
            return 0.0
    ...
    for k, a in kwargs.items():
        if k == "t0":
            ...
            try:
                ...
                raise Exception("tpolynomial profile must have one coefficient per order")
...
def LaserGaussian3D(box_side="xmin", a0=1., omega=1., focus=None, waist=3., incidence_angle=[0., 0.],
    ...
    if incidence_angle == [0., 0.]:
        ...
        invW = 1. / waist
        alpha = omega * Zr
        cy = cos(incidence_angle[0]);
        sy = sin(incidence_angle[0])
        cz = cos(incidence_angle[1]);
        sz = sin(incidence_angle[1])
        cycz = cy * cz;
        cysz = cy * sz;
        sycz = sy * cz;
        sysz = sy * sz
...
def LaserEnvelopeGaussian3D(a0=1., omega=1., focus=None, waist=3., time_envelope=tconstant(),
    ...
    def gaussian_beam_with_temporal_profile(x, y, z, t):
        ...
        exponential_with_total_phase = exp(1j * (phase - arctan((x - focus[0]) / Zr)))
        invWaist2 = (w / waist) ** 2
        spatial_amplitude = a0 * polarization_amplitude_factor * w * exp(
            -invWaist2 * ((y - focus[1]) ** 2 + (z - focus[2]) ** 2))
        space_time_envelope = spatial_amplitude * vectorize(time_envelope)(t)
        return space_time_envelope * exponential_with_total_phase
...
try:
    ...
    def LaserOffset(box_side="xmin", space_time_profile=[], offset=0., fft_time_window=None,
                    extra_envelope=lambda *a: 1., keep_n_strongest_modes=100, angle=0., number_of_processes=None,
                    file=None):
        L = Laser(
            box_side=box_side,
            file="none",
...
def _smilei_check():
    ...
    if len(Checkpoints) == 1 and Checkpoints.restart_dir:
        if len(Checkpoints.restart_files) == 0:
            ...
            files = filter(lambda a: smilei_mpi_rank == int(search(r'dump-[0-9]*-([0-9]*).h5$', a).groups()[-1]),
                           glob(pattern))

            if Checkpoints.restart_number is not None:
                # pick those file that match the restart_number
                files = filter(
                    lambda a: Checkpoints.restart_number == int(search(r'dump-([0-9]*)-[0-9]*.h5$', a).groups()[-1]),
                    files)
...
def _keep_python_running():
    ...
    if len(MovingWindow) > 0 or len(LoadBalancing) > 0:
        for s in Species:
            profiles += [s.number_density, s.charge_density, s.particles_per_cell,
                         s.charge] + s.mean_velocity + s.temperature
